...
def theCulling(minions):
	minions.sort()
	totalNum = int(((g.pop-1)*(g.pop-2))/2) #sum of 0 to g.pop, used to get weights equal to 1
	weights = [j/totalNum for j in range(g.pop-1)] #creates an array of weights, giving one to each minion
...
def newGen():
	...
	for i in range(g.muta):
		...
		x = minions[seed][1] + (random.random()-0.5) #subtracting by 0.5 changes range of epsilon to -0.5 to 0.5
		y = minions[seed][2] + (random.random()-0.5)
		z = fxy(x, y)
		minions.append([z, x, y])
...
with open(g.source+'/linearMaxes.csv', 'a') as file:
        mywriter = csv.writer(file, delimiter=',')
